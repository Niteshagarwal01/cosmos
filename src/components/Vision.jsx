...
#   MUTATION    →  β(t) = β₀·e^(Δfitness)`
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting) setFormulaKey(k => k + 1) },
      { threshold: 0.3 }
    )
    obs.observe(el)
  ...
  return (
    <section id="vision" style={{ padding: '0', background: '#000' }}>
      ...
      {/* Section label bar */}
      <div className="fade-in" style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: '20px', padding: '22px 80px', borderBottom: '1px solid #161616', borderTop: '1px solid #161616', overflow: 'hidden' }}>
        ...
        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#c8ff0088', letterSpacing: '0.25em', textTransform: 'uppercase', textShadow: '0 0 12px #c8ff0055' }}>02 — Vision</span>
        <div style={{ flex: 1, height: '1px', background: 'linear-gradient(90deg, #c8ff0022, transparent)' }} />
        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: '#333', letterSpacing: '0.2em' }}>UNIFIED CROSS-DOMAIN SIMULATION</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '420px 1fr', gap: '1px', background: '#0d0d0d' }}>
        ...
        <div style={{ background: '#020202', display: 'flex', flexDirection: 'column' }}>
          ...
          <div ref={formulaRef} className="fade-in" style={{ padding: '32px 44px', flex: 1 }}>
            ...
            <pre key={formulaKey} style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', lineHeight: 1.9, margin: 0, whiteSpace: 'pre' }}>
              {formula.split('\n').map((line, i) => {
                ...
                return (
                  <span key={i} className="formula-line" style={{ display: 'block', color: isKey ? '#c8ff00' : isRule ? '#1a1a1a' : isHeader ? '#252525' : '#3d3d3d', animationDelay: `${i * 42}ms` }}>
                    {line}
                  </span>
                )
              })}
        ...
        <div style={{ background: '#040404', display: 'flex', flexDirection: 'column' }}>
          ...
          <div className="fade-in stagger-reveal" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', background: '#0d0d0d', flex: 1 }}>
            {principles.map(({ id, title, desc }) => (
              <div key={id} className="principle-card" style={{ background: '#050505', padding: '28px 32px' }}>
                ...
                <p className="glow-text" style={{ fontSize: '12px', color: '#444', lineHeight: 1.7, margin: 0 }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
